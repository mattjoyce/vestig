...
### Source Abstraction
Unify provenance under a Source type:
- Current: `File → Chunk → Memory`
- Proposed: `Source{type='file'|'session'} → Chunk → Memory`
...
Memories connect to Entities via MENTIONS edges. This provides an alternative retrieval pathway independent of the provenance chain - critical for ad-hoc memories.
...
1. Should `memory add` require explicit entity specification to avoid orphans?
2. How aggressive should orphan cleanup be? Prune vs. archive vs. flag?
